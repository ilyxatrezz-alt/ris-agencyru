import { ArrowRight, Play, Star, CheckCircle, MapPin, Zap, Shield, TrendingUp } from "lucide-react";
...
const CityHero = () => {
  ...
  const title = isLocalPage && city 
    ? city.heroTitle 
    : "Превращаем рекламу в прибыль";
  
  const subtitle = isLocalPage && city 
    ? city.heroSubtitle 
    : "Создаём сайты и запускаем рекламу, которая окупается. Комплексный digital-маркетинг с гарантией результата.";

  const badge = isLocalPage && city 
    ? `${city.uniqueSellingPoint} • С 2014 года` 
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            ...
            >
              ...
              {isLocalPage && city && (
                ...
                >
                  <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-zinc-800/80 border border-zinc-700 text-zinc-300 text-sm font-medium">
              ...
              >
                {isLocalPage && city ? (
                  <h1 className="text-4xl sm:text-5xl md:text-6xl lg:text-6xl font-black leading-[0.95] tracking-tight">
                    <span className="text-gradient-primary">{city.heroTitle}</span>
                  </h1>
                ) : (
            ...
            >
              ...
              >
                {isLocalPage && city ? `Топ-1 в ${city.nameLocative}` : "Топ-1 агентство"}
              </motion.div>
